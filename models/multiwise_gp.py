...
from utils.assorted_utils import consolidate_duplicates
...
class MultiwiseGP(Model, GP, FantasizeMixin):
    """
    GP model for multinomial choice data: each trial has m candidates,
    ...
    def __init__(
        ...
        comparisons: Tensor,
        choices: Tensor,
        likelihood: Optional[MultinomialLogitLikelihood] = None,
        ...
        **kwargs,
        ...
        Args:
            ...
            comparisons: A `batch_shape x m x T` training comparisons;
            choices: A batch_shape x m training choice from the comparisons set
                comparisons[i] is a noisy indicator suggesting the utility value
                of comparisons[i, 0]-th is greatest.
        ...
        self.likelihood = (
            MultinomialLogitLikelihood() if likelihood is None else likelihood
        )
        ...
        self.unconsolidated_choices = None
        self.consolidated_choices = None
        ...
        self.set_train_data(datapoints, comparisons, choices, update_model=False)
        ...
        if self.datapoints is not None and self.comparisons is not None and self.choices is not None:
    ...
    def __deepcopy__(self, memo) -> MultiwiseGP:
        attrs = (
            "consolidated_datapoints",
            "consolidated_comparisons",
    ...
    def _grad_posterior_f(
        ...
        Args:
            ...
            comparisons: A Tensor of shape `batch_size x m x T`
            choices: Transpose of D. A Tensor of shape `batch_size x m`
        ...
        b = self.likelihood.negative_log_gradient_sum(utility=utility, comparisons=comparisons, choices=choices)
    ...
    def _hess_posterior_f(
        ...
        hl = self.likelihood.negative_log_hessian_sum(utility=utility, comparisons=comparisons, choices=choices)
        hess = hl + covar_inv
        return hess.reshape(hess.size(-2), hess.size(-1)).numpy() if ret_np else hess    
    ...
    def _update(self, datapoints: Tensor, **kwargs) -> None:
        ...
        # if len(datapoints.shape) == 2:  # (n, d)
        #     datapoints = datapoints.unsqueeze(0)  # (1, n, d)
        # if len(self.comparisons.shape) == 2:
        #     comparisons = self.comparisons.unsqueeze(0)
        # if len(self.choices.shape) == 1:
        #     choices = self.choices.unsqueeze(0)
        ...
        with torch.no_grad():
            ...
            if self._x0 is None or torch.Size(self._x0.shape) != init_x0_size:
                ...
                win_count = torch.zeros(n, device=datapoints.device)
                # indices of chosen datapoints
                choice_onehot = torch.nn.functional.one_hot(choices, num_classes=n).to(comparisons)

                ### new - 12/25/2025
                first_col = comparisons[:, 0:1].long()
                choice_onehot.scatter_(dim=1, index=first_col, value=1.0)
                remaining_cols = comparisons[:, 1:].long()
                choice_onehot.scatter_(dim=1, index=remaining_cols, value=-1.0)
                win_count = choice_onehot.sum(dim=-2).detach().cpu().numpy()
                wc_mean, wc_std = (
            ...
            if len(self.batch_shape) > 0:
                ...
                comparisons_v = comparisons.view(-1, self.m, self.T).cpu()
                choices_v = choices.view(-1, self.m).cpu()
                ch_v = self.covar_chol.view(-1, self.n, self.n).cpu()
                ci_v = self.covar_inv.view(-1, self.n, self.n).cpu()
                x = np.empty(x0.shape)
                for i in range(x0.shape[0]):
                    fsolve_args = (dp_v[i], comparisons_v[i], choices_v[i], ch_v[i], ci_v[i], True)
        ...
        self.likelihood_hess = self.likelihood.negative_log_hessian_sum(
            utility=f, comparisons=comparisons, choices=choices
        )
    ...
    def _util_newton_updates(self, dp, x0, max_iter=1, xtol=None) -> Tensor:
        ...
        while i < max_iter and diff > xtol:
            hl = self.likelihood.negative_log_hessian_sum(utility=x, comparisons=comparisons, choices=choices)
            ...
            g = self._grad_posterior_f(x, dp, comparisons, choices, ch, ci)
    ...
    def _consolidate_duplicates(
        self, datapoints: Tensor, comparisons: Tensor, choices: Tensor
    ) -> Tuple[Tensor, Tensor, Tensor]:
        """Consolidate and cache datapoints, choices and comparisons"""
        # check if consolidated datapoints/comparisons/choices are cached
        if (
            (datapoints is not self.unconsolidated_datapoints)
            or (comparisons is not self.unconsolidated_comparisons)
            or (choices is not self.unconsolidated_choices)
            or (self.consolidated_datapoints is None)
            or (self.consolidated_comparisons is None)
            or (self.consolidated_choices is None)
        ):
            self.unconsolidated_datapoints, self.unconsolidated_comparisons, self.unconsolidated_choices = (
                datapoints,
                comparisons,
                choices,
            )

            if len(datapoints.shape) > 2 or len(comparisons.shape) > 2 or len(choices.shape) > 1:
                ...
                self.consolidated_choices = choices
                self.consolidated_indices = None
            else:
                (
                    self.consolidated_datapoints,
                    self.consolidated_comparisons,
                    self.consolidated_choices,
        ...
        return self.consolidated_datapoints, self.consolidated_comparisons, self.consolidated_choices
    ...
    @property
    def choices(self) -> Tensor:
        r"""Alias for consolidated choices"""
        return self.consolidated_choices
    ...
    def set_train_data(
        ...
        r"""Set datapoints, choices and comparisons and update model properties if needed
        ...
        Args:
            ...
            comparisons: A tensor of size `batch_shape x m x T`. 
            choices: A tensor of size `batch_shape x m`. 
            ...
            update_model: True if we want to refit the model (see _update) after
                re-setting the data.
        """
        # When datapoints and/or choices, comparisons are None, we are constructing
        # a prior-only model
        if datapoints is None or comparisons is None or choices is None:
            return
        ...
        # if comparisons is not None and choices is not None:
        #     if strict:
        #         _check_strict_input([comparisons, choices], [self.train_targets], "targets")
        ...
        #     self.train_targets = (comparisons, choices)
        ...
        if comparisons is not None and choices is not None:
            ...
            # For gpytorch compatibility: make train_targets *some* tensor
            # (doesn't matter what, since MLL will call likelihood.log_p with
            # self.comparisons, self.choices explicitly)
            self.train_targets = comparisons
            # self.train_dat = datapoints

        if datapoints.ndim == 3 and datapoints.shape[0] == 1:
            datapoints = datapoints.squeeze(0)

        if comparisons.ndim == 3 and comparisons.shape[0] == 1:
            comparisons = comparisons.squeeze(0)

        if choices.ndim == 2 and choices.shape[0] == 1:
            choices = choices.squeeze(0)

        # if len(choices.shape) == 1:
        #     choices = choices.unsqueeze(0)
        
        # if len(comparisons.shape) == 2:
        #     comparisons = comparisons.unsqueeze(0)

        # self.datapoints and self.comparisons are being updated here
        self._consolidate_duplicates(datapoints, comparisons, choices)
        ...
        self.T = self.comparisons.shape[-1] # num of choices to pick from
        ...
        comp_view = self.comparisons.view(-1, self.m, self.T).long()
    ...
    def forward(self, datapoints: Tensor) -> MultivariateNormal:
        ...
        if self.training:
            ...
            self.set_train_data(
                ...
                choices=self.unconsolidated_choices,
                update_model=True,
            )
    ...
    def condition_on_observations(self, X: Tensor, Y: Tensor, Z: Tensor, **kwargs: Any) -> Model:
        ...
        Args:
            ...
            Y: A tensor of size `batch_shape x m x T`.
            Z: A tensor of size `batch_shape x m`.

        Returns:
            A (deepcopied) `Model` object of the same type, representing the
            original model conditioned on the new observations `(X, Y, Z)`.
        """
        new_model = deepcopy(self)

        if self._has_no_data():
            # If the model previously has no data, set X, Z and Y as the data directly
            new_model.set_train_data(X, Y, Z, update_model=True)
        else:
            # Can only condition on comparisons instead of the directly
            # observed values. Raise a RuntimeError if Y,Z is not a tensor presenting
            # pairwise comparisons
            if Y.dtype in (float32, float64) or Y.shape[-1] != self.T:
                raise RuntimeError(
                    "Conditioning on non-pairwise comparison observations."
                )

            # Reshaping datapoints and comparisons, choices by batches
            ...
            new_choices = self.choices.expand(
                Y_new_batch_shape + self.choices.shape[-1:]
            )
            ...
            shifted_choices = Z.to(new_choices)
            new_comparisons = torch.cat((new_comparisons, shifted_comp), dim=-2)
            new_choices = torch.cat((new_choices, shifted_choices), dim=-1)

            # TODO: be smart about how we can update covar matrix here
            new_model.set_train_data(new_datapoints, new_comparisons, new_choices, update_model=True)

        return new_model


class MultiwiseLaplaceMarginalLogLikelihood(MarginalLogLikelihood):
    ...
    def forward(self, post: Posterior, comp: Tensor) -> Tensor:
        ...
        log_likelihood = likelihood.log_p(utility=f_map, comparisons=model.comparisons, choices=model.choices)
...
class MultiwiseLikelihood(Likelihood, ABC):
    """
    Multirwise likelihood base class for pairwise preference GP (e.g., PairwiseGP).
    ...
    def forward(self, utility: Tensor, comparisons: Tensor, choices: Tensor,**kwargs: Any) -> Categorical:
        ...
        """
        return Categorical(probs=self.p(utility=utility, comparisons=comparisons, choices=choices))

    @abstractmethod
    def p(self, utility: torch.Tensor, comparisons: torch.Tensor) -> torch.Tensor:
        """
        """

    def log_p(self, utility: torch.Tensor, comparisons: torch.Tensor, choices: torch.Tensor) -> torch.Tensor:
        """return the log of p"""
        return torch.log(self.p(utility=utility, comparisons=comparisons, choices=choices))

    def negative_log_gradient_sum(self, utility: torch.Tensor, comparisons: torch.Tensor, choices: torch.Tensor) -> torch.Tensor:
        """
        """
        raise NotImplementedError

    def negative_log_hessian_sum(self, utility: torch.Tensor, comparisons: torch.Tensor, choices: torch.Tensor) -> torch.Tensor:
        """
        """
        raise NotImplementedError

class MultinomialLogitLikelihood(MultiwiseLikelihood):
    """
    Multinomial (softmax) likelihood for m-way choices.

    Given utilities f(x_1),...,f(x_T), the probability of choosing item i
    is softmax(f(x_i)) over the T candidates.
    """
    ...
    @staticmethod
    def _gather_candidate_utils(utility: torch.Tensor, comparisons: torch.LongTensor) -> torch.Tensor:
        """
        utility: (..., n)
        comparisons: (..., m, T)
        returns cand_utils: (..., m, T)
        """
        comparisons = comparisons.to(device=utility.device)
        # Expand utility to (..., m, n)
        expanded = utility.unsqueeze(-2).expand(*comparisons.shape[:-2], comparisons.size(-2), utility.size(-1))
        return torch.gather(expanded, dim=-1, index=comparisons)


    def log_p(self, utility: torch.Tensor, comparisons: torch.Tensor, choices: torch.Tensor) -> torch.Tensor:
        """
        Args:
            utility: Tensor of shape (..., n) = latent utilities f(x)
            comparisons: Tensor of shape (..., m, T) with candidate indices for each trial
            choices: Tensor of shape (..., m) with the chosen candidate index (0..T-1)

        Returns:
            log-probabilities, shape (..., m)
        """
        cand_utils = self._gather_candidate_utils(utility, comparisons)  # (m,T)

        log_probs = torch.log_softmax(cand_utils, dim=-1)                 # (m,T)

        # identify chosen candidate positions
        chosen_mask = (comparisons == choices.unsqueeze(-1))              # (m,T)

        # safety check (optional but recommended)
        if not torch.all(chosen_mask.sum(dim=-1) == 1):
            raise RuntimeError("Each choice must appear exactly once in comparisons.")
        chosen_log_probs = (log_probs * chosen_mask).sum(dim=-1)          # (m)

        return chosen_log_probs.sum(dim=-1)                                # (1,)

    def p(self, utility: torch.Tensor, comparisons: torch.Tensor, choices: torch.Tensor) -> torch.Tensor:
        """
        Probability distribution over candidates for each trial.
        Returns softmax distribution (..., m, T).
        """
        return torch.exp(self.log_p(utility, comparisons, choices))

    def negative_log_gradient_sum(self, utility: torch.Tensor, comparisons: torch.Tensor, choices: torch.Tensor) -> torch.Tensor:
        """
        Gradient of -log p wrt utilities, summed over trials.

        Returns:
            Tensor of shape (..., n) matching utility
        """ 
        cand_utils = self._gather_candidate_utils(utility, comparisons)  # (..., m, T)
        probs = torch.softmax(cand_utils, dim=-1)            # (..., m, T)
        # One-hot encode choices: (..., m, T)
        chosen_mask = (comparisons == choices.unsqueeze(-1)).type_as(probs)  # (m,T)
        # Gradient wrt cand_utils: probs - one_hot (..., m, T)
        contrib = probs - chosen_mask
        # Scatter back into full utility space (..., n)
        grad_full = torch.zeros_like(utility)
        grad_full.scatter_add_(-1, comparisons.reshape(*comparisons.shape[:-2], -1), contrib.reshape(*contrib.shape[:-2], -1))
        return grad_full  # already summed over m

    def negative_log_hessian_sum(
        self,
        utility: torch.Tensor,          # (batch..., n)
        comparisons: torch.LongTensor,  # (batch..., m, T)
        choices: torch.LongTensor = None # unused
    ) -> torch.Tensor:
        """
        Vectorized assembly of the full Hessian (..., n, n) from per-trial blocks,
        WITHOUT Python loops. Uses flattened-index scatter_add trick.

        Returns:
            hess_full: Tensor of shape (..., n, n)
        """
        m, T = comparisons.shape
        B = 1
        N = utility.size(-1)

        cand_utils = self._gather_candidate_utils(utility, comparisons)
        probs = torch.softmax(cand_utils, dim=-1)

        W = torch.diag_embed(probs) - probs.unsqueeze(-1) * probs.unsqueeze(-2)

        idx_row = comparisons.unsqueeze(-1).expand(B, m, T, T)
        idx_col = comparisons.unsqueeze(-2).expand(B, m, T, T)

        K = m * T * T
        idx_row_f = idx_row.reshape(B, K)
        idx_col_f = idx_col.reshape(B, K)
        W_f = W.reshape(B, K)

        H = torch.zeros(B, N, N, device=utility.device, dtype=utility.dtype)

        for b in range(B):
            H[b].index_put_(
                (idx_row_f[b], idx_col_f[b]),
                W_f[b],
                accumulate=True
            )
    ...
    # def negative_log_hessian_sum(self, utility: torch.Tensor, comparisons: torch.Tensor, choices: torch.Tensor) -> torch.Tensor:
    #     """
    #     Hessian of -log p wrt utilities, summed over trials.
    ...
    #     cand_utils = torch.gather(expanded, dim=-1, index=comparisons) # (..., m, T)
    #     probs = torch.softmax(cand_utils, dim=-1)            # (..., m, T)

    #     # Covariance matrix of softmax distribution per trial: diag(p) - p p^T
    #     eye = torch.eye(probs.size(-1), device=utility.device, dtype=utility.dtype)
    #     cov = probs.unsqueeze(-1) * (eye - probs.unsqueeze(-2))  # (..., m, T, T)

    #     # Scatter into global Hessian
    #     n = utility.size(-1)
    #     hess_full = torch.zeros(*utility.shape[:-1], n, n, device=utility.device, dtype=utility.dtype)

    #     idx_i = comparisons.unsqueeze(-1).expand(*comparisons.shape, comparisons.shape[-1])  # (..., m, T, T)
    #     idx_j = comparisons.unsqueeze(-2).expand(*comparisons.shape, comparisons.shape[-1])  # (..., m, T, T)

    #     hess_full.scatter_add_(-2, idx_i, torch.zeros_like(hess_full).scatter(-1, idx_j, cov))  # tricky part
...
# class MultinomialLogitLikelihood(torch.nn.Module):
#     """Multinomial choice likelihood (softmax over m candidates per trial)."""

#     def log_p(self, utility: torch.Tensor, choice: torch.Tensor) -> torch.Tensor:
#         """
#         Args:
#             utility: (batch, m) tensor of latent utilities
#             choice:  (batch,) tensor of chosen indices in [0, m-1]
#         Returns:
#             log_prob: (batch,) log-likelihoods per trial
#         """
#         log_probs = torch.nn.functional.log_softmax(utility, dim=-1)  # (batch, m)
#         return log_probs.gather(-1, choice.unsqueeze(-1)).squeeze(-1)

#     def p(self, utility: torch.Tensor) -> torch.Tensor:
#         """Return probabilities for each choice (softmax)."""
#         return torch.nn.functional.softmax(utility, dim=-1)  # (batch, m)

#     def negative_log_gradient(self, utility: torch.Tensor, choice: torch.Tensor) -> torch.Tensor:
#         """Gradient of -log p wrt utility, per trial."""
#         p = torch.nn.functional.softmax(utility, dim=-1)                  # (batch, m)
#         onehot = torch.nn.functional.one_hot(choice, num_classes=utility.size(-1)).type_as(p)
#         return p - onehot                               # (batch, m)

#     def negative_log_hessian(self, utility: torch.Tensor) -> torch.Tensor:
#         """Hessian of -log p wrt utility, per trial."""
#         p = torch.nn.functional.softmax(utility, dim=-1)                  # (batch, m)
#         diag_p = torch.diag_embed(p)                    # (batch, m, m)
#         outer_p = p.unsqueeze(-1) @ p.unsqueeze(-2)     # (batch, m, m)
#         return diag_p - outer_p                         # (batch, m, m)